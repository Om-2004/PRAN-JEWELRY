...
function KaragirForm() {
  ...
  const [formData, setFormData] = useState({
    metalType: '', ornamentName: '', labourCharge: '',
    karatorHUID: '', grossWeight: '', netWeight: '',
    subtype: '', karagirName: '', status: '', remarks: '',
    purity: '', grams: '' // fields for 'out' action
  });
  const vendor = JSON.parse(localStorage.getItem('vendor')) || {};

  const handleSubmit = (e) => {
    e.preventDefault();
    // Include actionType in payload
    axios.post('/api/karagir', { actionType, ...formData, vendorId: vendor.id })
      .then(res => alert('Karagir entry submitted'))
    ...
    <form onSubmit={handleSubmit}>
      ...
      {actionType === 'in' && (
        <div>
          ...
          />
          <input type="text" placeholder="Ornament Name"
          ...
          />
          <input type="text" placeholder="Karator HUID"
          ...
          />
          <input type="text" placeholder="Karagir Name"
